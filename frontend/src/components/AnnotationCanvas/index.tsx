...
import { render_annotation_elements, render_drawing_preview, render_collaboration_layer, update_cursor_style } from './render'
...
export default function annotation_canvas({
	...
	const [preview_point, set_preview_point] = useState<{ x: number; y: number } | undefined>(undefined)
	...
	const finish_polygon = useCallback(
		() => finish_polygon_logic(
			drawing_polygon, set_drawing_polygon, set_preview_point,
			image, active_class, annotations, on_annotations_change, set_selected_ann_id
		),
	...
	const handle_wheel = useCallback(
		(e: Konva.KonvaEventObject<WheelEvent>) => {
			...
			if (e.evt.ctrlKey || e.evt.metaKey) {
				...
				const new_scale = e.evt.deltaY > 0
	...
	const handle_mouse_down = useCallback(
		(e: Konva.KonvaEventObject<MouseEvent>) => {
			...
			handle_mouse_down_logic(
				active_tool, pos, set_selected_ann_id, set_selected_prediction_id,
				drawing_polygon, set_drawing_polygon, finish_polygon,
				zoom_level, image, set_is_drawing, set_drawing_start,
				set_drawing_rect, set_drawing_mask_lines, brush_size
			)
		},
		[active_tool, get_pointer_pos_in_image, set_selected_ann_id, drawing_polygon, zoom_level, image, brush_size]
	)
	...
	const handle_mouse_move = useCallback(
		() => {
			...
			handle_mouse_move_logic(
				active_tool, pos, is_drawing, drawing_start, image,
				set_drawing_rect, drawing_polygon, set_preview_point, set_drawing_mask_lines
			)
		},
		[active_tool, is_drawing, drawing_start, get_pointer_pos_in_image, image, drawing_polygon.length]
	)

	const handle_mouse_up = useCallback(
		() => handle_mouse_up_logic(
			is_drawing, active_tool, drawing_rect, image, active_class,
			annotations, on_annotations_change, set_selected_ann_id,
			set_is_drawing, set_drawing_rect, drawing_mask_lines, set_drawing_mask_lines
		),
		[is_drawing, active_tool, drawing_rect, image, active_class, annotations, on_annotations_change, set_selected_ann_id, drawing_mask_lines]
	)
	...
	return (
		...
		>
			...
			>
				...
				{render_mask_layer(image, annotations, show_predictions, predictions, drawing_mask_lines, get_class_color, active_tool, active_class, brush_opacity)}
				<Layer>
					{image && render_annotation_elements(annotations, image, selected_ann_id, hovered_ann_id, zoom_level, active_tool, get_class_color, get_class_name, collaborators, set_selected_ann_id, set_hovered_ann_id, on_annotations_change)}
					{image && show_predictions && render_annotation_elements(predictions, image, selected_prediction_id, hovered_ann_id, zoom_level, active_tool, get_class_color, get_class_name, collaborators, set_selected_prediction_id, set_hovered_ann_id, on_predictions_change, true)}
					{render_drawing_preview(is_drawing, drawing_rect, active_tool, drawing_polygon, preview_point, zoom_level, get_class_color, get_class_name, brush_size, active_class)}
				</Layer>
