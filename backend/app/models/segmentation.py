...
class SegmentationMask(Base):
    ...
    project_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("projects.id"), index=True, nullable=False
    )
    image_id: Mapped[str] = mapped_column(String(64), index=True, nullable=False)
    mask_data: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # COCO RLE encoded JSON/String
    bbox_prompt: Mapped[str] = mapped_column(
        String(128), nullable=False
    )  # JSON representation of [x1, y1, x2, y2]
