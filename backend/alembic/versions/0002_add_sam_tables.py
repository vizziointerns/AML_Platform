...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("datasetCount", sa.Integer(), nullable=False, server_default="0"),
        sa.Column(
            "annotationProgress", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column("members", sa.Text(), nullable=False, server_default="[]"),
        sa.Column("lastUpdated", sa.Integer(), nullable=False, server_default="0"),
        sa.Column(
            "created_at", sa.String(length=32), nullable=False, server_default=""
        ),
        sa.Column(
            "task_type", sa.String(length=16), nullable=False, server_default="detect"
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("project_id", "image_id"),
    )
    op.create_index(
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_segmentation_masks_image_id"), table_name="segmentation_masks"
    )
    op.drop_index(
        op.f("ix_segmentation_masks_project_id"), table_name="segmentation_masks"
    )
